...
for (let i = 0; i < items.length; i++) {
    ...
    // AUDIO 1.3x (EN UNA SOLA LÍNEA ⚠️)
...
`;
    ...
    // =========================
    execSync(`
ffmpeg -y -loop 1 -i image_${i}.jpg -vf "scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:(ow-iw)/2:(oh-ih)/2,zoompan=z='if(lte(on,12),1.5-(on*0.04),1)':d=125:s=720x1280,eq=contrast=1.2:saturation=1.3,unsharp=5:5:1.0,setsar=1" -t 5 -c:v libx264 -preset ultrafast -crf 28 -pix_fmt yuv420p image_part_${i}.mp4
`, { stdio: 'inherit' });
    ...
    execSync(`
ffmpeg -y -i video_${i}.mp4 -vf "setpts=PTS/1.5,scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:(ow-iw)/2:(oh-ih)/2,setsar=1" -t ${remaining} -an -c:v libx264 -preset ultrafast -crf 28 -pix_fmt yuv420p video_part_${i}.mp4
`, { stdio: 'inherit' });
